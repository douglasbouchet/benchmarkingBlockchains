...
providerToSmartContractAddress = {
    "TicketProvider": "0xbce1b23c7544422f1e2208d29a6a3aa9fabab250",
...
def processJson(_json, counter):
    ...
    for i, tx in enumerate(_json["result"]):
        ...
        data.append(
            {
                "id": counter,
                "timeStamp": int(tx["timeStamp"], base=16),
...
def fetchAllData():
    ...
    for i in range(startBlock, endBlock, blockCheckPerQuery):
        print("Fetching data from block {} to {}".format(i, i + blockCheckPerQuery))
        res = getTxs(
            i,
            i + blockCheckPerQuery,
            "0xbce1b23c7544422f1e2208d29a6a3aa9fabab250",
            username,
            apiKey,
        )
...
def fetchAllDataV2():
    ...
    while currentBlock < endBlock:
        print(
            "Fetching from block {} to {}".format(currentBlock, currentBlock + nBlocks)
        )
        res = getTxs(
            currentBlock, currentBlock + nBlocks, smartContractAddress, username, apiKey
        )
        data = processJson(res, counter)
        nTxs = len(data)
