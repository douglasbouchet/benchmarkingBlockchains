...
workloadName = "nftix"
loaded = json.load(open("data/data_mint_20363134_20470000.json"))
...
with open("counter.yaml") as file:
    ...
    for index, row in df.iterrows():
        data["workloads"][0]["client"]["behavior"][0]["load"][
            int(row["timeStamp"])
        ] = int(row["count"])
        data["workloads"][0]["client"]["behavior"][0]["load"][
            int(row["timeStamp"]) + 1
...
print(
    "Don't forget tu update the contract name and method called inside the generated workload"
)
